<template>
  <section class="layout layout-grid-scroll-rows">
    <div class="col images-grid">
      <OptimizedImage
        ...
        :alt="image.alt || title"
        sizes="(min-width: 1600px) 50vw, (min-width: 1280px) 55vw, (min-width: 1024px) 70vw, 90vw"
      />
    </div>
    <div class="col text">
      <h3>{{ title }}</h3>
...
const richTextBlocks = computed(() => {
  if (!Array.isArray(props.text)) return []
  return props.text
    .map((block = {}) => {
      const ideas = ideaKeys
        .map((key) => {
          ...
          return { label: key, text: value }
        })
        .filter(Boolean)
      return { description: block?.description || '', ideas }
    })
